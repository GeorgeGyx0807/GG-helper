...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".poppy").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".poppy" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "poppy" / "agent" / "welcome" / "screen"
    ...
    assert "MINI CODING AGENT" not in welcome
    assert "P O P P Y" in welcome
    assert "local desktop and coding assistant" in welcome
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "poppy/0.1"
...
def test_dashscope_anthropic_client_uses_bearer_authentication():
    captured = {}
    ...
    class FakeResponse:
        ...
        def read(self):
            return json.dumps({"content": [{"type": "text", "text": "<final>好</final>"}]}).encode()
    ...
    with patch("urllib.request.urlopen", fake_urlopen):
        assert client.complete("你好", 64) == "<final>好</final>"

    assert captured["url"] == "https://dashscope.aliyuncs.com/apps/anthropic/v1/messages"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "poppy.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("poppy.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = poppy_pkg.build_agent(args)
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"POPPY_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "poppy.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("poppy.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_defaults_provider_to_deepseek(tmp_path):
    args = poppy_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider == "deepseek"


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = poppy_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = poppy_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "poppy.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("poppy.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("poppy.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            poppy_pkg.build_agent(args)
    ...
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                ...
                "POPPY_DEEPSEEK_API_KEY=sk-project-deepseek",
...
def test_build_agent_uses_deepseek_provider_by_default(tmp_path):
    ...
    with patch.dict(
        ...
        {
            "POPPY_DEEPSEEK_API_BASE": "https://api.deepseek.com/anthropic",
            "POPPY_DEEPSEEK_API_KEY": "sk-test",
            "POPPY_DEEPSEEK_MODEL": "deepseek-v4-pro",
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .poppy/.\n"
    ...
    index_path = tmp_path / ".poppy" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".poppy" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".poppy" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .poppy/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".poppy" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".poppy" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(poppy_pkg.__file__).as_posix().endswith("/poppy/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(poppy_pkg.main)
    assert callable(poppy_pkg.build_agent)
    assert callable(poppy_pkg.build_arg_parser)
