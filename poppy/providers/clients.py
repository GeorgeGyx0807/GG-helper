...
OPENAI_COMPATIBLE_USER_AGENT = "poppy/0.1"
...
class OpenAICompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `Poppy.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
...
def _is_dashscope_anthropic(base_url):
    host = str(base_url).lower()
    return "dashscope.aliyuncs.com" in host or ".maas.aliyuncs.com" in host


def _anthropic_headers(base_url, api_key, *, stream=False):
    """Build the authentication headers required by the selected backend.

    DashScope's Anthropic-compatible endpoint follows its documented
    ``Authorization: Bearer`` contract. DeepSeek and the existing generic
    Anthropic-compatible gateways use ``x-api-key``.
    """
    headers = {
        "Content-Type": "application/json",
    ...
    if _is_dashscope_anthropic(base_url):
...
def _anthropic_http_error(base_url, status, body):
    if status in {401, 403}:
        provider = "阿里云百炼" if _is_dashscope_anthropic(base_url) else "当前模型服务"
        return RuntimeError(
            f"模型鉴权失败（HTTP {status}）：请在设置中为{provider}重新保存与当前接口匹配的 API Key，然后点击“测试连接”。"
        )
    if status == 404:
        return RuntimeError("模型请求失败（HTTP 404）：模型名称与 API 地址不匹配，请检查模型设置。")
    return RuntimeError(f"模型请求失败（HTTP {status}）：{body}")
...
class AnthropicCompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        headers = _anthropic_headers(self.base_url, self.api_key)
        ...
        for attempt in range(attempts):
            try:
                ...
                if exc.code >= 500 and attempt < attempts - 1:
                    time.sleep(0.5 * (attempt + 1))
                    continue
                raise _anthropic_http_error(self.base_url, exc.code, body) from exc
            except (urllib.error.URLError, RemoteDisconnected) as exc:
                if attempt < attempts - 1:
                    time.sleep(0.5 * (attempt + 1))
                    continue
    ...
    def complete_stream(
        ...
        request = urllib.request.Request(
            self.base_url + "/messages",
            data=json.dumps(payload).encode("utf-8"),
            headers=_anthropic_headers(self.base_url, self.api_key, stream=True),
            method="POST",
        )
        attempts = 3
        for attempt in range(attempts):
