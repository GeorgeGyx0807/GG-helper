...
class AssistantService:
    ...
    def start_static_run(self, agent, user_message, answer):
        session_id = agent.session["id"]
        run_id = agent.new_run_id()
        token = CancellationToken()
        ...
        thread = Thread(
            target=self._run_static,
            args=(run, agent, str(user_message), str(answer)),
            name=f"poppy-{run_id}",
            daemon=True,
        )
        run.thread = thread
    ...
    def _run_static(self, run, agent, user_message, answer):
        run.status = "running"
        try:
            agent.record({"role": "user", "content": user_message, "created_at": now()})
            agent.emit_event(
                "run.started",
                {"task_id": "", "user_request": user_message[:300]},
                run_id=run.run_id,
            )
            run.cancellation_token.raise_if_cancelled()
            agent.record({"role": "assistant", "content": answer, "created_at": now()})
            run.answer = answer
            run.status = "completed"
            agent.emit_event("message.completed", {"content": answer}, run_id=run.run_id)
            agent.emit_event(
                "run.completed",
                {"status": "completed", "stop_reason": "evidence_insufficient", "final_answer": answer},
                run_id=run.run_id,
            )
        except RunCancelled:
            run.status = "cancelled"
            run.answer = "Run cancelled."
            agent.emit_event("run.cancelled", {"status": "cancelled"}, run_id=run.run_id)
        except Exception as exc:
            run.status = "failed"
            run.error = str(exc)
            agent.emit_event("run.failed", {"status": "failed", "error": str(exc)}, run_id=run.run_id)
    ...
    def cancel_session(self, session_id):
        """Cancel the active run in a session when its authorization is revoked."""
        with self._lock:
            run_id = self._active_sessions.get(session_id)
