...
class ContextManager:
    ...
    def build(self, user_message):
        ...
        在 agent 链路里的位置：
        它位于 `Poppy.ask()` 的每轮模型调用之前，是“真正发请求给模型”
        ...
        if not context_reduction_enabled:
            ...
            if checkpoint_text and "Task checkpoint:" not in prompt:
        ...
        prompt = self._assemble_prompt(rendered)
        if checkpoint_text and "Task checkpoint:" not in prompt:
            # Checkpoint state is an explicit recovery contract, not optional
            # history.  Keep it after reduction even when the prefix budget is
            # saturated by a large tool registry.
            prompt = prompt.rstrip() + "\n\n" + checkpoint_text
        reduction_log = []
