...
Poppy 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class Poppy:
    def __init__(
        ...
        self.library_searcher = library_searcher
        self.personal_memory_sink = personal_memory_sink
        self._event_sequences = defaultdict(int)
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".poppy" / "runs")
    ...
    def promote_durable_memory(self, user_message, final_answer):
        ...
        if promoted and self.personal_memory_sink is not None:
            self.personal_memory_sink(promoted, self.session.get("id", ""))
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Poppy.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Poppy.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Poppy.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Poppy.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Poppy.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Poppy.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Poppy.retry_notice()
        if "<final>" in raw:
            final = Poppy.extract(raw, "final").strip()
            if "<tool" in final:
                return "retry", Poppy.retry_notice("model mixed an internal tool call into the final answer")
            if final:
                return "final", final
            return "retry", Poppy.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if "<tool" in raw:
            return "retry", Poppy.retry_notice("model returned an unparseable tool trace")
        if raw:
            return "final", raw
        return "retry", Poppy.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Poppy.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Poppy.extract_raw(body, key)
